...
EPUB 就係一個 ZIP 檔案，入面裝住網頁內容。

具體嚟講：

| 檔案 | 做乜 |
|------|------|
| mimetype | 告訴閱讀器「我係 EPUB」（8 bytes，固定寫法） |
| container.xml | 指路牌：內容喺邊個資料夾 |
| content.opf | 清單：列出所有檔案 + 書名作者語言 |
| *.html | 書嘅內容（每章一頁） |
| *.css | 樣式（字型、行距、排版） |
| *.jpg / *.png | 圖片 |
| toc.ncx | 目錄（閱讀器用嚟跳頁） |

重點：唔同閱讀器（Apple Books、Kobo、Kindle）都識開呢個 ZIP，攞到入面嘅內容，跟 manifest 同 spine 排好畀你睇。呢個就係 EPUB 嘅威力 — 寫一次，到處睇。

### 正式定義

正式講：**EPUB = 一班 HTML 頁面 + CSS 樣式 + 圖片，包喺一個 ZIP 入面，加埋一張「清單」同「閱讀順序」。**
...
具體入面有咩檔案、每個檔案做咩 — 下個 module 拆開一個 EPUB 慢慢講。
