...
> **Predict**: Before reading deeper: what do you expect happens when motion.div interacts with motion.svg in animation — framer motion?
>
> *Answer: The system relies on motion.div to keep motion.svg predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats motion.div as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind motion.div are violated. The fix: treat it as part of the contract of animation — framer motion, not an optimization.*
