...
```mermaid
mindmap
  root((Data Fetching TanStack Query))
...
> **Predict**: Before reading deeper: what do you expect happens when placeholderdata: keeppreviousdata interacts with keeppreviousdata: true in data fetching — tanstack query?
>
> *Answer: The system relies on placeholderdata: keeppreviousdata to keep keeppreviousdata: true predictable — when both apply, the stricter rule wins.*
> **Cloze**: {blank} governs how data fetching — tanstack query behaves when multiple keeppreviousdata: true concerns collide.
> **Cloze**: The rule that keeps placeholderdata: keeppreviousdata correct under load is called {blank}.
> **Cloze**: In data fetching — tanstack query, skiptoken determines {blank}.
> **Spot the Mistake**: A developer treats placeholderdata: keeppreviousdata as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind placeholderdata: keeppreviousdata are violated. The fix: treat it as part of the contract of data fetching — tanstack query, not an optimization.*
