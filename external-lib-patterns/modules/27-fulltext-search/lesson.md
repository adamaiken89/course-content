...
```mermaid
mindmap
  root((Full-Text Search Algolia InstantSearch))
    Algolia Architecture
    InstantSearch Setup
    Hooks API
    Faceting Strategy
    Example 1 Multi-Facet Filter Sidebar
    Example 2 Custom SearchBox with Recent Searches and Aut
```
...
> **Predict**: Before reading deeper: what do you expect happens when <mark> interacts with algolia architecture in full-text search — algolia instantsearch?
>
> *Answer: The system relies on <mark> to keep algolia architecture predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats <mark> as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind <mark> are violated. The fix: treat it as part of the contract of full-text search — algolia instantsearch, not an optimization.*
