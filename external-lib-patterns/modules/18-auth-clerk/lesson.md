...
> **Predict**: Commit to an answer: does authentication — clerk get simpler or harder once gettoken() enters the picture?
>
...
> **Cloze**: {blank} governs how authentication — clerk behaves when multiple authorization: bearer <token> concerns collide.
> **Cloze**: The rule that keeps gettoken() correct under load is called {blank}.
> **Cloze**: In authentication — clerk, clerk architecture determines {blank}.
> **Spot the Mistake**: Code review note: someone applies authorization: bearer <token> everywhere "to be safe" in a authentication — clerk codebase. Spot the mistake.
>
> *Answer: Blanket application hides which spots actually need authorization: bearer <token>. Apply it where the semantics demand it, and document why.*
