...
> **Predict**: Before reading deeper: what do you expect happens when date-fns-tz interacts with getnative() in date/time libraries?
>
> *Answer: The system relies on date-fns-tz to keep getnative() predictable — when both apply, the stricter rule wins.*
> **Cloze**: {blank} governs how date/time libraries behaves when multiple getnative() concerns collide.
> **Cloze**: The rule that keeps date-fns-tz correct under load is called {blank}.
> **Cloze**: In date/time libraries, formatintimezone determines {blank}.
> **Spot the Mistake**: A developer treats date-fns-tz as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind date-fns-tz are violated. The fix: treat it as part of the contract of date/time libraries, not an optimization.*
