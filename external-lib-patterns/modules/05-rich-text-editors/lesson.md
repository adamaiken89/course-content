...
```mermaid
mindmap
  root((Rich Text Editors TipTap & Lexical))
...
> **Predict**: Before reading deeper: what do you expect happens when editordocument interacts with onchange in rich text editors — tiptap & lexical?
>
> *Answer: The system relies on editordocument to keep onchange predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats editordocument as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind editordocument are violated. The fix: treat it as part of the contract of rich text editors — tiptap & lexical, not an optimization.*
