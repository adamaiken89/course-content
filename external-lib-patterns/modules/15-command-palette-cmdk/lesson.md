...
> **Predict**: Before reading deeper: what do you expect happens when shouldfilter interacts with role="combobox" in command palette — cmdk?
>
> *Answer: The system relies on shouldfilter to keep role="combobox" predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats shouldfilter as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind shouldfilter are violated. The fix: treat it as part of the contract of command palette — cmdk, not an optimization.*
