...
```mermaid
mindmap
  root((Data Grids TanStack Table & AG Grid))
    Grid Landscape
...
> **Predict**: Before reading deeper: what do you expect happens when appcolumndef interacts with appcolumndef.extra in data grids — tanstack table & ag grid?
>
> *Answer: The system relies on appcolumndef to keep appcolumndef.extra predictable — when both apply, the stricter rule wins.*
> **Cloze**: {blank} governs how data grids — tanstack table & ag grid behaves when multiple appcolumndef.extra concerns collide.
> **Cloze**: The rule that keeps appcolumndef correct under load is called {blank}.
> **Cloze**: In data grids — tanstack table & ag grid, record<string, unknown> determines {blank}.
> **Spot the Mistake**: A developer treats appcolumndef as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind appcolumndef are violated. The fix: treat it as part of the contract of data grids — tanstack table & ag grid, not an optimization.*
