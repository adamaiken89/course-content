...
```mermaid
mindmap
  root((Toast & Notifications Sonner))
    Sonner Architecture
...
> **Predict**: Before reading deeper: what do you expect happens when <toaster /> interacts with toast.success() in toast & notifications — sonner?
>
> *Answer: The system relies on <toaster /> to keep toast.success() predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats <toaster /> as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind <toaster /> are violated. The fix: treat it as part of the contract of toast & notifications — sonner, not an optimization.*
