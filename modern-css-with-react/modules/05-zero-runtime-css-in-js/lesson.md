...
```mermaid
mindmap
  root((Zero-Runtime CSS-in-JS Vanilla Extract))
...
> **Predict**: Commit to an answer: does zero-runtime css-in-js (vanilla extract) get simpler or harder once .css enters the picture?
>
...
> **Cloze**: {blank} governs how zero-runtime css-in-js (vanilla extract) behaves when multiple .css.ts concerns collide.
> **Cloze**: The rule that keeps .css correct under load is called {blank}.
> **Cloze**: In zero-runtime css-in-js (vanilla extract), .d.ts determines {blank}.
> **Spot the Mistake**: Code review note: someone applies .css.ts everywhere "to be safe" in a zero-runtime css-in-js (vanilla extract) codebase. Spot the mistake.
>
> *Answer: Blanket application hides which spots actually need .css.ts. Apply it where the semantics demand it, and document why.*
