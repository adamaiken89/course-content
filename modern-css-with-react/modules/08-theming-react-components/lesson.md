...
```mermaid
mindmap
  root((Theming React Components with CSS))
...
> **Predict**: Before reading deeper: what do you expect happens when @scope interacts with var(--name) in theming react components with css?
>
> *Answer: The system relies on @scope to keep var(--name) predictable — when both apply, the stricter rule wins.*
> **Cloze**: {blank} governs how theming react components with css behaves when multiple var(--name) concerns collide.
> **Cloze**: The rule that keeps @scope correct under load is called {blank}.
> **Cloze**: In theming react components with css, var(--color-primary) determines {blank}.
> **Spot the Mistake**: A developer treats @scope as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind @scope are violated. The fix: treat it as part of the contract of theming react components with css, not an optimization.*
