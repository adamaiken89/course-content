...
```mermaid
mindmap
  root((Plain CSS & Sass in React))
    Global CSS in React The Problem
    BEM as Namespacing Strategy
    Sass in React What Works What Doesnt
...
> **Predict**: Before reading deeper: what do you expect happens when .card interacts with button--primari in plain css & sass in react?
>
> *Answer: The system relies on .card to keep button--primari predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats .card as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind .card are violated. The fix: treat it as part of the contract of plain css & sass in react, not an optimization.*
