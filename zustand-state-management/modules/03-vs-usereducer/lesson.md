...
> **Predict**: Before reading deeper: what do you expect happens when usereducer interacts with set() in zustand vs usereducer: reducers without boilerplate?
>
> *Answer: The system relies on usereducer to keep set() predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats usereducer as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind usereducer are violated. The fix: treat it as part of the contract of zustand vs usereducer: reducers without boilerplate, not an optimization.*
