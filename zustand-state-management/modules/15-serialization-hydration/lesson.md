...
```mermaid
mindmap
  root((Weaknesses 2 and 3 Serialization Hydration Gotchas))
    Serialization Weakness JSONs Type Erasure
    ...
    Example 1 Fixing a Broken Date Serialization in Product
    Example 2 Next.js App SSR Hydration Mismatch with Persi
```
...
> **Predict**: Before reading deeper: what do you expect happens when json.stringify interacts with date in weaknesses 2 and 3 — serialization, hydration gotchas?
>
> *Answer: The system relies on json.stringify to keep date predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats json.stringify as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind json.stringify are violated. The fix: treat it as part of the contract of weaknesses 2 and 3 — serialization, hydration gotchas, not an optimization.*
