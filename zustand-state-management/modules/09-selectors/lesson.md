...
```mermaid
mindmap
  root((Selector Architecture Granular Subscriptions Optimizati))
    ...
    Example 1 Fixing Selector Anti-patterns Before and Afte
...
> **Predict**: Before reading deeper: what do you expect happens when usestore interacts with object.is in selector architecture — granular subscriptions, optimization?
>
> *Answer: The system relies on usestore to keep object.is predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats usestore as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind usestore are violated. The fix: treat it as part of the contract of selector architecture — granular subscriptions, optimization, not an optimization.*
