...
```mermaid
mindmap
  root((Public API vs Internal API Store Encapsulation))
    The Encapsulation Problem in Zustand
    Public API vs Internal API The Boundary
    The Prefix Convention
    Barrel Exports as API Contract
    Example 1 Refactoring Internal State Without Breaking C
    Example 2 Composing Selectors from Multiple Internal So
```
...
> **Predict**: Commit to an answer: does public api vs internal api — store encapsulation get simpler or harder once create() enters the picture?
>
...
> **Cloze**: {blank} governs how public api vs internal api — store encapsulation behaves when multiple private concerns collide.
> **Cloze**: The rule that keeps create() correct under load is called {blank}.
> **Cloze**: In public api vs internal api — store encapsulation, _refreshtimer determines {blank}.
> **Spot the Mistake**: Code review note: someone applies private everywhere "to be safe" in a public api vs internal api — store encapsulation codebase. Spot the mistake.
>
> *Answer: Blanket application hides which spots actually need private. Apply it where the semantics demand it, and document why.*
