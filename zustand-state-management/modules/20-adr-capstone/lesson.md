...
```mermaid
mindmap
  root((Capstone Architecture Decision Record))
    What Is an Architecture Decision Record Why State Manag
    Evaluating State Solutions Decision Matrix
    Store Architecture Documentation Boundaries Ownership A
    Store Map
      Performance Decision Atomic Stores vs Monolithic Store
      Testing Strategy Documentation What to Test at Store vs
    Testing Strategy
      Migration ADR Documenting Migration Plan Rollback Crite
    Context
```
...
> **Predict**: Commit to an answer: does capstone — architecture decision record get simpler or harder once src/platform/stores/auth-store.ts enters the picture?
>
...
> **Spot the Mistake**: Code review note: someone applies src/cart/stores/cart-store.ts everywhere "to be safe" in a capstone — architecture decision record codebase. Spot the mistake.
>
> *Answer: Blanket application hides which spots actually need src/cart/stores/cart-store.ts. Apply it where the semantics demand it, and document why.*
