...
```mermaid
mindmap
  root((Derived State Computed Values Selector Composition))
    Derived State Concept Compute Dont Store
    ...
    Example 1 E-Commerce Product Catalog with Derived Filte
...
> **Predict**: Before reading deeper: what do you expect happens when items: todo[] interacts with totalitems: number in derived state — computed values, selector composition?
>
> *Answer: The system relies on items: todo[] to keep totalitems: number predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats items: todo[] as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind items: todo[] are violated. The fix: treat it as part of the contract of derived state — computed values, selector composition, not an optimization.*
