...
```mermaid
mindmap
  root((Zustand with Server Components Client Store Boundaries))
    Stores Are Client-Only The use client Rule
    Passing Initial State from Server to Client Hydration P
    Server Data Flows RSC Fetches Client Consumes with Zust
    Per-Request Store vs Singleton Store
    Example 1 Migrating a Zustand Cart from Client-Only to 
...
> **Predict**: Before reading deeper: what do you expect happens when use client interacts with usestore in zustand with server components — client store boundaries?
>
> *Answer: The system relies on use client to keep usestore predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats use client as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind use client are violated. The fix: treat it as part of the contract of zustand with server components — client store boundaries, not an optimization.*
