...
```mermaid
mindmap
  root((Zustand vs useContext Re-Renders Provider Hell))
    ...
    Example 1 Real-Time Dashboard Context vs Zustand Perfor
    Example 2 Migrating a Settings Panel from Context to Zu
```
...
> **Predict**: Before reading deeper: what do you expect happens when value interacts with value={state} in zustand vs usecontext: re-renders, provider hell?
>
> *Answer: The system relies on value to keep value={state} predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats value as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind value are violated. The fix: treat it as part of the contract of zustand vs usecontext: re-renders, provider hell, not an optimization.*
