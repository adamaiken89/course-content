...
```mermaid
mindmap
  root((Middleware Immer Persist Custom Chains))
...
> **Predict**: Commit to an answer: does middleware — immer, persist, custom chains get simpler or harder once api enters the picture?
>
...
> **Spot the Mistake**: Code review note: someone applies . middleware reads everywhere "to be safe" in a middleware — immer, persist, custom chains codebase. Spot the mistake.
>
> *Answer: Blanket application hides which spots actually need . middleware reads. Apply it where the semantics demand it, and document why.*
