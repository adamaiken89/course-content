...
```mermaid
mindmap
  root((Zustand vs useState When Local State Isnt Enough))
    Limits of useState
    When useState Is Enough
    Refactoring useState to Zustand
    Zustand for Cross-Tree State
    Example 1 Refactoring a Dashboard with Prop Drilling Pa
...
> **Predict**: Before reading deeper: what do you expect happens when usestate interacts with usecartstore() in zustand vs usestate — when local state isnt enough?
>
> *Answer: The system relies on usestate to keep usecartstore() predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats usestate as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind usestate are violated. The fix: treat it as part of the contract of zustand vs usestate — when local state isnt enough, not an optimization.*
