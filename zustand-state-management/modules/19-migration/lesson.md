...
```mermaid
mindmap
  root((Migration useReducer Context Redux Toolkit to Zustand))
    Migration Strategy Incremental Not Big Bang
    Strategy Spectrum
    useReducer Zustand Extract Reducer Logic into Store Act
    Context Zustand Replace Provider + useContext with useS
    Example 1 Migrating useReducer + Context Todo App
    Example 2 Migrating Redux Toolkit Dashboard with Async 
```
...
> **Predict**: Before reading deeper: what do you expect happens when set() interacts with usereducer in migration — usereducer, context, redux toolkit to zustand?
>
> *Answer: The system relies on set() to keep usereducer predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats set() as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind set() are violated. The fix: treat it as part of the contract of migration — usereducer, context, redux toolkit to zustand, not an optimization.*
