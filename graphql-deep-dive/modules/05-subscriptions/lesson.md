...
> **Predict**: Before reading deeper: what do you expect happens when connection_init interacts with subscribe in subscriptions?
>
> *Answer: The system relies on connection_init to keep subscribe predictable — when both apply, the stricter rule wins.*
> **Cloze**: {blank} governs how subscriptions behaves when multiple subscribe concerns collide.
> **Cloze**: The rule that keeps connection_init correct under load is called {blank}.
> **Cloze**: In subscriptions, resolve determines {blank}.
> **Spot the Mistake**: A developer treats connection_init as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind connection_init are violated. The fix: treat it as part of the contract of subscriptions, not an optimization.*
