...
> **Predict**: Commit to an answer: does federation design get simpler or harder once email: string! enters the picture?
>
...
> **Cloze**: {blank} governs how federation design behaves when multiple email: int! concerns collide.
> **Cloze**: The rule that keeps email: string! correct under load is called {blank}.
> **Cloze**: In federation design, string! determines {blank}.
> **Spot the Mistake**: Code review note: someone applies email: int! everywhere "to be safe" in a federation design codebase. Spot the mistake.
>
> *Answer: Blanket application hides which spots actually need email: int!. Apply it where the semantics demand it, and document why.*
