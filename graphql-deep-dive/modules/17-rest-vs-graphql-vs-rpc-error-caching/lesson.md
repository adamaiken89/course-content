...
```mermaid
mindmap
  root((REST vs GraphQL vs RPC Error & Caching))
    Error Models
    Partial Success
    Caching Semantics
    Cache Invalidation
...
> **Predict**: Commit to an answer: does rest vs graphql vs rpc: error & caching get simpler or harder once errors[] enters the picture?
>
...
> **Spot the Mistake**: Code review note: someone applies data everywhere "to be safe" in a rest vs graphql vs rpc: error & caching codebase. Spot the mistake.
>
> *Answer: Blanket application hides which spots actually need data. Apply it where the semantics demand it, and document why.*
