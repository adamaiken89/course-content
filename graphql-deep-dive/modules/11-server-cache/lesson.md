...
> **Predict**: Before reading deeper: what do you expect happens when ?query=...&variables=... interacts with s-maxage in server cache?
>
> *Answer: The system relies on ?query=...&variables=... to keep s-maxage predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats ?query=...&variables=... as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind ?query=...&variables=... are violated. The fix: treat it as part of the contract of server cache, not an optimization.*
