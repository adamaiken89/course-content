...
> **Predict**: Before reading deeper: what do you expect happens when $name: type interacts with $limit: int = 10 in queries?
>
> *Answer: The system relies on $name: type to keep $limit: int = 10 predictable — when both apply, the stricter rule wins.*
> **Cloze**: {blank} governs how queries behaves when multiple $limit: int = 10 concerns collide.
> **Cloze**: The rule that keeps $name: type correct under load is called {blank}.
> **Cloze**: In queries, @skip determines {blank}.
> **Spot the Mistake**: A developer treats $name: type as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind $name: type are violated. The fix: treat it as part of the contract of queries, not an optimization.*
