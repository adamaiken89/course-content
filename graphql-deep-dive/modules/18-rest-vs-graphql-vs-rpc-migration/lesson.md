...
```mermaid
mindmap
  root((REST vs GraphQL vs RPC Migration & TCO))
...
> **Predict**: Commit to an answer: does rest vs graphql vs rpc: migration & tco get simpler or harder once get /users/42 enters the picture?
>
...
> **Spot the Mistake**: Code review note: someone applies { user(id:42) { name } } everywhere "to be safe" in a rest vs graphql vs rpc: migration & tco codebase. Spot the mistake.
>
> *Answer: Blanket application hides which spots actually need { user(id:42) { name } }. Apply it where the semantics demand it, and document why.*
