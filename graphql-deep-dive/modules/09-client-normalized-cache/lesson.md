...
> **Predict**: Before reading deeper: what do you expect happens when __typename interacts with keyfields in client normalized cache?
>
> *Answer: The system relies on __typename to keep keyfields predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats __typename as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind __typename are violated. The fix: treat it as part of the contract of client normalized cache, not an optimization.*
