...
```mermaid
mindmap
  root((REST vs GraphQL vs RPC Data Fetching))
    Over-Fetching
...
> **Predict**: Commit to an answer: does rest vs graphql vs rpc: data fetching get simpler or harder once /users/1 enters the picture?
>
...
> **Spot the Mistake**: Code review note: someone applies — dashboard only needs everywhere "to be safe" in a rest vs graphql vs rpc: data fetching codebase. Spot the mistake.
>
> *Answer: Blanket application hides which spots actually need — dashboard only needs. Apply it where the semantics demand it, and document why.*
