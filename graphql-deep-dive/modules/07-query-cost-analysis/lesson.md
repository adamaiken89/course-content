...
> **Predict**: Before reading deeper: what do you expect happens when total = σ (fieldweight × multiplierarg) interacts with users.complexity = 5 × 50 = 250 in query cost analysis?
>
> *Answer: The system relies on total = σ (fieldweight × multiplierarg) to keep users.complexity = 5 × 50 = 250 predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats total = σ (fieldweight × multiplierarg) as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind total = σ (fieldweight × multiplierarg) are violated. The fix: treat it as part of the contract of query cost analysis, not an optimization.*
