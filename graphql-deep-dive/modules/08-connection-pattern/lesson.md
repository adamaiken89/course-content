...
> **Predict**: Before reading deeper: what do you expect happens when ?page=3&limit=10 interacts with offset in connection pattern?
>
> *Answer: The system relies on ?page=3&limit=10 to keep offset predictable — when both apply, the stricter rule wins.*
> **Cloze**: {blank} governs how connection pattern behaves when multiple offset concerns collide.
> **Cloze**: The rule that keeps ?page=3&limit=10 correct under load is called {blank}.
> **Cloze**: In connection pattern, where cursor > x determines {blank}.
> **Spot the Mistake**: A developer treats ?page=3&limit=10 as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind ?page=3&limit=10 are violated. The fix: treat it as part of the contract of connection pattern, not an optimization.*
