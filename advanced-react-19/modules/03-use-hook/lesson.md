...
```mermaid
mindmap
  root((use Hook Reading Promises and Context in Render))
...
> **Predict**: Before reading deeper: what do you expect happens when use(promise) interacts with use(context) in use() hook — reading promises and context in render?
>
> *Answer: The system relies on use(promise) to keep use(context) predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats use(promise) as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind use(promise) are violated. The fix: treat it as part of the contract of use() hook — reading promises and context in render, not an optimization.*
