...
```mermaid
mindmap
  root((Server Components Composition Client Boundaries Directi))
    Server Components vs Client Components Mental Model Shi
...
> **Predict**: Commit to an answer: does server components: composition, client boundaries, directives get simpler or harder once bind enters the picture?
>
...
> **Cloze**: {blank} governs how server components: composition, client boundaries, directives behaves when multiple async concerns collide.
> **Cloze**: The rule that keeps bind correct under load is called {blank}.
> **Cloze**: In server components: composition, client boundaries, directives, await determines {blank}.
> **Spot the Mistake**: Code review note: someone applies async everywhere "to be safe" in a server components: composition, client boundaries, directives codebase. Spot the mistake.
>
> *Answer: Blanket application hides which spots actually need async. Apply it where the semantics demand it, and document why.*
