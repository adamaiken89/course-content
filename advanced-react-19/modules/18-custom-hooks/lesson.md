...
```mermaid
mindmap
  root((Custom Hooks Patterns Composition Refactoring Testing))
    ...
    Example 2 Refactoring a Heavy Component into Testable H
```
...
> **Predict**: Before reading deeper: what do you expect happens when use interacts with | one output — in custom hooks patterns — composition, refactoring, testing?
>
> *Answer: The system relies on use to keep | one output — predictable — when both apply, the stricter rule wins.*
> **Spot the Mistake**: A developer treats use as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind use are violated. The fix: treat it as part of the contract of custom hooks patterns — composition, refactoring, testing, not an optimization.*
