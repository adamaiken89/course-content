...
```mermaid
mindmap
  root((Transitions Deep useTransition startTransition Batching))
...
> **Predict**: Before reading deeper: what do you expect happens when starttransition interacts with usetransition in transitions deep — usetransition, starttransition, batching?
>
> *Answer: The system relies on starttransition to keep usetransition predictable — when both apply, the stricter rule wins.*
> **Cloze**: {blank} governs how transitions deep — usetransition, starttransition, batching behaves when multiple usetransition concerns collide.
> **Cloze**: The rule that keeps starttransition correct under load is called {blank}.
> **Cloze**: In transitions deep — usetransition, starttransition, batching, usedeferredvalue determines {blank}.
> **Spot the Mistake**: A developer treats starttransition as optional because "it works without it." Where is the mistake?
>
> *Answer: It works only until the assumptions behind starttransition are violated. The fix: treat it as part of the contract of transitions deep — usetransition, starttransition, batching, not an optimization.*
